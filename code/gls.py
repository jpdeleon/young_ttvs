...
class Gls:
    ...
    def _assignTimeSeries(self, lc):
        ...
        if isinstance(lc, (tuple, list, np.ndarray)):
            ...
            if len(lc) in (2, 3):
                ...
                raise (
                    ValueError(
                        "lc is a list or tuple with "
                        + str(len(lc))
                        + " elements. Needs to have 2 or 3 elements."
    ...
    def _buildFreq(self):
        ...
        if self.f is None:
            ...
            if self.fend <= self.fbeg:
                raise (
                    ValueError(
                        "fend is smaller than (or equal to) fbeg but it must be larger. "
    ...
    def pnorm(self, norm="ZK"):
        ...
        if norm == "Scargle":
            ...
            power = (self.N - 3) / 2.0 * p / (1.0 - self.p.max())
        elif norm == "chisq":
            power = self._YY * self.wsum * (1.0 - p)
            self.label["ylabel"] = r"$\chi^2$"
        elif norm == "wrms":
            power = sqrt(self._YY * (1.0 - p))
    ...
    def _peakPeriodogram(self):
        ...
        self.rms = rms = sqrt(self._YY * (1.0 - pmax))
        ...
        p["ph"] = ph = arctan2(self._a[k], self._b[k]) / (2.0 * pi)
        p["T0"] = self.t.min() - ph / fbest
        p["offset"] = self._off[k] + self._Y  # Re-add the mean.

        # Error estimates
        p["e_amp"] = sqrt(2.0 / self.N) * rms
        p["e_ph"] = e_ph = sqrt(2.0 / self.N) * rms / amp / (2.0 * pi)
        p["e_T0"] = e_ph / fbest
        p["e_offset"] = sqrt(1.0 / self.N) * rms
    ...
    def plot(
        ...
        if gls:
            ...
            plt.plot(
                1 / self.f if period else self.f,
                self.power,
                "b-",
                linewidth=0.5,
            )
            ...
            plt.plot(
                1 / fbest if period else fbest,
                self.power[self.p.argmax()],
                "r.",
                label="$1/f = %f$" % (1 / fbest),
            )

            x2tics = 1 / np.array([0.5, 1, 2, 3, 5, 10, 20.0, 100])
            mx2tics = 1 / np.array([0.75, 1.5, 2.5, 4, 15, 40, 60.0, 80, 100])
    ...
    def probInv(self, Prob):
        ...
        if self.norm == "Cumming":
            return (self.N - 3) / 2 * (Prob ** (-2.0 / (self.N - 3)) - 1)
        if self.norm == "wrms":
        ...
        if self.norm == "ZK":
            return p
        if self.norm == "lnL":
            return -0.5 * self._YY * self.wsum * (1.0 - p) - 0.5 * np.sum(
                np.log(2 * np.pi * self.e_y ** 2)
            )
        if self.norm == "dlnL":
            return 0.5 * self._YY * self.wsum * p
    ...
    def powerLevel(self, FAPlevel):
        ...
        """
        Prob = 1.0 - (1.0 - FAPlevel) ** (1.0 / self.M)
